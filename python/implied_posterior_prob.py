...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb  5 14:41:55 2020
...
# %%
#creating the datasets georgi wants


dfDataScaledTrain = pd.concat([x_cvtrain, y_cvtrain], axis =1)
dfDataScaledTest  = pd.concat([x_test, y_test], axis =1)
...
print('Step1')   
    # set manually
myclf = 'RF' # 'xgboost', 'RF' , 'LinearSVC', 'SVC', 'Logistic', 'GaussianNB', 'Ada', 'lda' , 'qda'
bootstr = 1 # set manually # number of bootstrap samples
...
for b in range(0,bootstr):
    ...
    if myclf == 'xgboost':
        ...
        n_estimators_RF = 100
        clfBase = RandomForestClassifier(n_estimators=n_estimators_RF, class_weight={0: thres_neg[i], 1:thres_pos[i]}, n_jobs=4,
        ...
        if use_rbf == 0:
            ...
            C_SVC = 2 # 10 (hastie = 20)
            gamma_SVC = 0.001 # 1 , 0.001   (hastie = 0.3)
            clfBase = svm.SVC(tol=0.0001, 
                              C=C_SVC,
                              gamma=gamma_SVC,
                              decision_function_shape='ovr', 
...
#    # option 2. Assign thres_pos and thres_neg - rough grid
    ...
    thres_neg = 1-thres_pos # np.arange(9.95,0.0,-0.05)
    #thres_pos = np.arange(0.9,0.0,-0.02)
...
for b in range(0,bootstr): # range(0,bootstr):
    ...
    for i in tqdm(range(0,len(thres_pos))):
        ...
        if myclf == 'xgboost': # note: only weight on positive examples is supported
            ...
            clf[i] = RandomForestClassifier(n_estimators=n_estimators_RF, class_weight={0: thres_neg[i], 1:thres_pos[i]}, n_jobs=4,
                                            oob_score=False, random_state=0, verbose=0, warm_start=False)
        elif myclf == 'LinearSVC': # LinearSVC
...
try:
    clf_probsBase_i_mean = np.mean(clf_probsBase_i_all)
    ...
    impliedPProb_i_mean = np.mean(newPoint_prob_i_all)
    impliedPProb_i_std = np.std(newPoint_prob_i_all) # should be percentile , as we cannot go outside [0,1]
except:
    pass
skipme = 1 # plot for a given i the "step 2" surface (the separation line after re-weighting)
if skipme == 0:
    if use_mesh == 1:
        b = 0
...
# predict the posterior probability for all new pointa (newPoint_prob_all)
if (myclf == 'LinearSVC') or (myclf == 'SVC'):
...
plt.figure(); plt.hist(newPoint_prob_all,bins=10); plt.title('Distribution of point according to postptob - BASE'); plt.xlim([0, 1])
plt.figure(); plt.hist(clf_probsBase[0],bins=10); plt.title('Distribution of point according to postptob - implied'); plt.xlim([0, 1])
...
if myclf == 'SVC':
    ...
    clf_probsBase_scaled = clf_probsBase[0]
wrk_pred = np.array([])
wrk_true = np.array([])
bin_size = np.array([])
...
plt.scatter(newPoint_prob_all, dfDataScaledTest.iloc[:,-1] + np.random.rand(32308)*0.2 - 0.1,marker='x',color='blue'); plt.ylim([-0.25, 1.25])
...
if myclf == 'SVC':
    plt.figure()
    plt.scatter(newPoint_prob_all,clf_SVC[0]); plt.xlabel('Implied posterior probability'); plt.ylabel('raw_SVM_score'); plt.grid() # plt.xlabel('newPoint_prob_all');
    plt.figure()
    plt.scatter(clf_probsBase[0],clf_SVC[0]); plt.xlabel('Platt probability'); plt.ylabel('raw_SVM_score'); plt.grid()
...
if len(np.unique(dfDataScaledTest.iloc[:,-1].values))>1:
    ...
    plt.title('ROC curve')
    plt.plot(fpr, tpr, 'b', label = '(org) AUC = %0.2f' % roc_auc)
    plt.axes().set_aspect('equal', 'box')
    ...
    roc_auc = metrics.auc(fpr, tpr)
    plt.plot(fpr, tpr, 'k', label = '(new) AUC = %0.2f' % roc_auc)
    plt.axes().set_aspect('equal', 'box')
    plt.legend(loc = 'lower right')
    plt.plot([0, 1], [0, 1],'r--')
    plt.xlim([0, 1])
...
plt.title('Raw classification scores on a equally-spaced postprob grid')
plt.xlabel('$\widehat{P(+|x)}$'); plt.grid()
plt.figure();plt.scatter(postprob, np.sign(t), color = 'black') # for each hyperplane among the hyperplanes associated wit
plt.title('Classification label $\in {-1,+1}$ on a equally-spaced postprob grid')
plt.xlabel('$\widehat{P(+|x)}$'); plt.grid()
...
implied_pr = pd.DataFrame(data = newPoint_prob_all, index = y_test.index, columns = ['implied_pr'])
implied_pr_iso = pd.DataFrame(data = y_, index = y_test.index, columns = ['Iso calibrated'])



#%%

implied_1 = pd.concat([implied_pr, implied_pr_iso], axis = 1)

model_predict_test = pd.read_excel(path+'/output/final_model_predictions_testv2.xlsx', index_col = 0)

rf_pred = model_predict_test[['RandomForestClassifier_pr_cal_iso']]

implied = pd.concat([implied_1, rf_pred], axis = 1)

classifiers = ['implied_pr',  'Iso calibrated', 'RandomForestClassifier_pr_cal_iso']
...
fop_extra, mpv_extra = calibration_curve(y_test, implied_pr['implied_pr'], n_bins=10)
# plot model reliability
plt.plot(mpv_extra, fop_extra, marker='.')

fop_extra, mpv_extra = calibration_curve(y_test, implied_pr_iso['Iso calibrated'], n_bins=10)
# plot model reliability
plt.plot(mpv_extra, fop_extra, marker='.')

fop_extra, mpv_extra = calibration_curve(y_test, implied['RandomForestClassifier_pr_cal_iso'], n_bins=10)
# plot model reliability
plt.plot(mpv_extra, fop_extra, marker='.')


plt.title('Calibration plots (reliability diagram)', fontweight='bold', fontsize=12)
plt.legend(['Perfect', 'implied','Iso calibrated', 'RF'], loc='upper left')
...
#%%
#lets calibrate those bitches


implied_pr['Target_slow3cm'] = y_test

implied_pr_cal = implied_pr.iloc
implied_pr_test = implied_pr.iloc
